<template>
	<div class="top_view">
		...
		<div class="projectTitle">基于Java的学院教学工作量统计系统</div>
...
<script setup>
	...
	const getSession = () => {
		context?.$http({
			...
			if(context?.$toolUtil.storageGet('sessionTable') == 'jiaoshi'){
				context?.$toolUtil.storageSet('adminHeadportrait',res.data.data.touxiang)
				adminHeadportrait.value = context.$config.url+res.data.data.touxiang
			}
		})
	}
	// 退出登录
